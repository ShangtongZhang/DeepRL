...
class BootstrappedNStepQRDQNAgent(BaseAgent):
    def __init__(self, config):
        ...
        self.active_options = self.network.tensor(np.random.choice(
            config.candidate_options, size=config.num_workers)).long()
    ...
    def iteration(self):
        ...
        for _ in range(config.rollout_length):
            ...
            if config.option_type in ['per_step', 'per_episode']:
                q_values = quantile_values[self.network.range(config.num_workers), :, self.active_options]
            else:
            ...
            for i, terminal in enumerate(terminals):
                if terminals[i]:
                    self.active_options[i] = self.network.tensor(np.random.choice(config.candidate_options))
