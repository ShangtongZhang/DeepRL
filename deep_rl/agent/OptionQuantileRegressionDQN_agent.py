...
class OptionQuantileRegressionDQNAgent(BaseAgent):
    ...
    def episode(self, deterministic=False):
        ...
        option = np.random.choice(self.config.option_candidates)
        total_reward = 0.0
        steps = 0
        while True:
            value = self.network.predict(np.stack([self.config.state_normalizer(state)])).squeeze(0).detach()
            ...
            if self.config.option_type == 'per_step' or self.config.option_type == 'per_episode':
                q_values = value[0, :, option]
            else:
                q_values = value.sum(-1)
