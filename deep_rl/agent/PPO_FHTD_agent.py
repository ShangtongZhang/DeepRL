...
class PPOFHTDAgent(BaseAgent):
    ...
    def augment_prediction(self, prediction):
        if self.config.aux:
            ...
            prediction['v'] = prediction['v_h'][:, [self.config.active]]
    ...
    def train_with_extra_data(self, storage):
        ...
        for _ in range(config.optimization_epochs):
            ...
            for batch_indices in sampler:
                ...
                next_v_h = next_prediction['v_h']
                next_v_h[:, -1] = 0
                next_v_h = torch.cat([next_v_h[:, [-1]], next_v_h[:, :-1]], dim=-1)
                target = sampled_rewards + config.discount * sampled_masks * next_v_h
                value_loss = (prediction['v_h'] - target).pow(2).mul(0.5).mean()
    ...
    def step(self):
        ...
        storage = Storage(config.rollout_length, ['v_canonical'])
        states = self.states
